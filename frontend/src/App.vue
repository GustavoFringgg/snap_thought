          ...
          <div class="page-header__left">
            ...
            <div class="week-selector">
              ...
              <div class="select-wrap">
                ...
                <svg class="select-icon" width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
              ...
              <div class="select-wrap">
                ...
                >
                  <option
                    ...
                    :key="w.index"
                    :value="w.index"
                  >{{ w.label }}{{ w.isCurrentWeek ? ' · 本週' : '' }}</option>
                </select>
                <svg class="select-icon" width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
                  <path d="M2 4l4 4 4-4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
        ...
        <div class="mobile-only">
          <div class="mobile-day-nav">
            <button
              ...
              :style="activeDay === day.key ? `--btn-color: var(${day.colorVar})` : ''"
              @click="handleChangeDay(day.key)"
            >
              {{ day.shortLabel }}
              <span v-if="isCurrentWeek && todayKey === day.key" class="mobile-today-dot"></span>
            </button>
          </div>
...
import type { DayData, DayKey, Note } from './types'
...
function findCurrentWeekIndex(weekList: WeekRange[]): number {
  const idx = weekList.findIndex(w => w.isCurrentWeek)
  return idx >= 0 ? idx : 0
}
...
const weeks = computed(() => generateWeeks(selectedYear.value))
const selectedWeekIndex = ref(findCurrentWeekIndex(weeks.value))

const selectedWeek = computed(() => weeks.value[selectedWeekIndex.value] ?? weeks.value[0])
const isCurrentWeek = computed(() => selectedWeek.value?.isCurrentWeek ?? false)
...
const DAY_DEFS: Omit<DayData, 'notes'>[] = [
  ...
  { key: 'tue', label: '星期二', shortLabel: '二', colorVar: '--color-tuesday' },
  { key: 'wed', label: '星期三', shortLabel: '三', colorVar: '--color-wednesday' },
  { key: 'thu', label: '星期四', shortLabel: '四', colorVar: '--color-thursday' },
...
const days = computed<DayData[]>(() =>
  DAY_DEFS.map(d => ({
    ...d,
    notes: notesStore[noteKey(selectedYear.value, selectedWeekIndex.value, d.key)] ?? [],
  }))
)
...
const activeDayData = computed(() => days.value.find(d => d.key === activeDay.value))
